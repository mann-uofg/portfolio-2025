...
export function AppleHelloEnglishEffect({
  ...
  // In this specific implementation, 'speed' acts as a duration multiplier.
  // Lower number = Faster animation.
  const calc = (x: number) => x * speed;

  return (
    <motion.svg
      className={`h-20 md:h-32 w-auto ${className || ''}`}
    ...
    >
      ...
      <motion.path
        ...
        animate={animateProps}
        transition={{
          duration: calc(4), // Increased from 2.8 to 4 to slow down "ello"
          ease: "easeInOut",
          delay: calc(0.7),
